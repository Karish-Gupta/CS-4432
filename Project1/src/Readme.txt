...
Enter "Circular" or "LRU". Circular is set as base algorithm, so if the algorithm is not typed properly, it will be set.

The program will then say: The program is ready for the next command (GET, SET, PIN, UNPIN).
For Get enter: Get <record ID>

For Set enter: Set <record ID> <"F01-Rec010, No Work, 31 Hill St, age100.">
The set content should be in quotes, however, this is not included in the 40 bytes set file.

For Pin enter: Pin <block ID>
For Unpin enter: Unpin <block ID>
